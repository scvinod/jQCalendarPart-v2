...
<script type="text/javascript">
	...
	(function($)
	{
		...
		function applyCalendarData(xData, status)
		{
			...
			$(".divDatePicker1 .anchCalLi").click(function() {
		...
		function processResult()
		{
			...
			$(calendarListItems.responseXML).SPFilterNode("z:row").each(function()
			{
				...
				$('.divDatePicker1 .ui-datepicker-calendar a')
				...
				if($('.divDatePicker1 .ui-datepicker-calendar a').first().parent('td').attr("data-year") == date.getFullYear() && $('.ui-datepicker-calendar a').first().parent('td').attr("data-month") == date.getMonth()){					
					...
					else if (date.getDate() == currentDate.getDate())
					{
						...
						$(".divDatePicker1 #" + date.getDate() + "_eventPopUp" + " .divCalendarUL").append(calliHtml);
					}
				}
				currentDate = date;
			});
		...
		function CreatePopUp(eventDate, eventDuration, itemURL, title)
		{
			...
			if ($(".divDatePicker1 #" + eventDate.getDate() + "_eventPopUp").html() != null)
			{
				$(".divDatePicker1 #" + eventDate.getDate() + "_eventPopUp").empty();
			}
			...
			$(".divDatePicker1 #" + eventDate.getDate() + "_eventPopUp").append("<h3 class='calHead'>" + eventDate.toLocaleDateString() + "</h3 >");
			$(".divDatePicker1 #" + eventDate.getDate() + "_eventPopUp").append("<ul class = 'divCalendarUL'>");
			$(".divDatePicker1 #" + eventDate.getDate() + "_eventPopUp" + " .divCalendarUL").append(calliHtml);
		}
...
<div class="divDatePicker1"></div>


<style type="text/css">
	.divDatePicker1 .eventPopUpDiv {
		Z-INDEX: 9002;
		PADDING-RIGHT: 10px;
	...
	.divDatePicker1 .CalendarLI {
		MARGIN-LEFT: -23px;
		FONT-WEIGHT: normal
	}
	
	.divDatePicker1 .divCalendarLI {
		MARGIN-LEFT: -23px
	}
	
	.divDatePicker1 .calHead {
		PADDING-LEFT: 4px;
		PADDING-RIGHT: 4px;
	...
	.divDatePicker1 .anchCalLi {

		TEXT-ALIGN: left !important;
	}
	
	.divDatePicker1 .anchCalLi:hover {
	...
	.divDatePicker1 .anchCalLi:visited {
		text-decoration: none;
		color: rgb(0, 114, 188) !important;
	}
	
	.divDatePicker1 .ui-state-default:visited {
		text-decoration: none;
		color: rgb(0, 114, 188) !important;
	}
	.divDatePicker1 .ui-widget-header .ui-icon {
